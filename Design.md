...
5. Engineering Components
   There are four engineering panels. Each of the panel names should be linked up with a function on the ship, and this should be hardcoded so that it is the same every game, and between both players. The functions on the ship that should be associated with a unique panel are; Weapons, Thrust, Fuel, and Power.
   Each panel should impact the relevant system based on the number of connections that are not correct. A single incorrect connection imparts a minor penalty, three incorrect connections impart a medium penalty, and five incorrect connections impart a heavy penalty. These counts and penalties should be defined as constants for each function so they are easily editable.
   Power: When the ship is calculating how much power to restore each game second, it should determine it based on the baseline recovery, how close to correct the pulse frequency control from the scienceStore is, and the penalty from the engineering panel associated with Power. If the ship is generating less than half as much power as expected, an orange alert should be shown to tell the user this.
   Thrust: When the ship is calculating how fast it should move (how much distance it should gain each tick), it should factor in the penalty from the engineering panel associated with Thrust
   Fuel: When the ship is calculating how much fuel it loses each second, it should factor in the penalty from the engineering panel associated with Fuel. Additionally, when calculating the cooldown on the 'Dump' and 'Refuel' buttons, the penalty from the engineering panel associated with Fuel should be used. If there's a light penalty, it should take 1.5 times as long. If there's a medium penalty, it should take 2 times as long. If there's a heavy penalty, it should take 3 times as long.
   Weapons: The weapon cooldowns should be impacted by the engineering panel associated with Weapons. Light; 1.5 times as long. Medium; 2 times as long. Heavy: 3 times as long.
...
- Each stage has different correct navigation values (pitch, yaw, roll) for both Albatross and Kestrel
