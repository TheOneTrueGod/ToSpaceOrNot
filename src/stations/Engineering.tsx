...
export const Engineering: React.FC = () => {
	...
	const renderClosedPanel = (panelName: string) => {
		...
		return (
			<div
				...
				className={`${isSchematic ? "bg-yellow-100" : "bg-gray-600"
					} border-2 border-gray-400 rounded-lg p-8 cursor-pointer hover:${isSchematic ? "bg-yellow-200" : "bg-gray-500"
			...
			>
				...
				<div className="flex items-center justify-center h-full">
					<div className="text-center">
						<span
							className={`${isSchematic ? "text-gray-800" : "text-gray-800"
						...
						{DEBUG_MODE ||
							(isSchematic && (
								<span
									className={`${isSchematic ? "text-gray-600" : "text-gray-700"
										} font-mono text-sm block mt-2`}
								>
									({getPanelSystemMapping(otherPlayer)[panelName]})
								</span>
							))}
	...
	const renderOpenPanel = (panelName: string) => {
		...
		return (
			<div
				key={panelName}
				className={`${isSchematic ? "bg-yellow-50" : "bg-gray-700"
				...
				>
					<div className="flex justify-between items-center mb-4">
						<div className="text-center mx-auto">
							<h3
								className={`${isSchematic ? "text-gray-800" : "text-white"
									} font-mono text-lg`}
							>
								{panelName}
							</h3>
							{DEBUG_MODE && (
								<span
									className={`${isSchematic ? "text-gray-600" : "text-gray-300"
										} font-mono text-sm block`}
								>
									({getPanelSystemMapping(isSchematic ? otherPlayer : currentPlayer)[panelName]})
								</span>
							)}
						</div>
					</div>
					...
					<div className="absolute left-8 top-8">
						<div
							className={`${isSchematic ? "text-gray-700" : "text-white"
						...
						{COLORS.map((color, index) => (
							<div
								key={`input-${index}`}
								className={`w-8 h-8 rounded-full border-2 ${isSchematic ? "border-gray-600" : "border-white"
									} mb-4 ${!isSchematic
